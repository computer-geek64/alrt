...
def predictPoint(latitudes, longitudes, radius):
    ...
    latest_predicted_location = latitudes[14]*weight+bias
    degrees_of_latlong = radius/69.172
    change_in_y = math.sqrt((degrees_of_latlong)**2 / (1+(1/(weight**2))))
    change_in_x = change_in_y / weight
    if (latitudes[0] < latitudes[14]):
        new_latitude = latitudes[14] + change_in_x
    else:
        new_latitude = latitudes[14] - change_in_x
    if (longitudes[0] < longitudes[14]):
        new_longitude = longitudes[14] + change_in_y
    else:
        new_longitude = longitudes[14] - change_in_y
...
# print(predictPoint(latitudes, longitudes, 5))